...
### #23 - Historical-Best Model Two-Run Deep Evaluation Completed (2026-07-21 16:54 -04:00)
...
- The approved design is `docs/superpowers/specs/2026-07-21-historical-best-two-run-deep-evaluation-design.md`; the measured implementation and results below complete that direction.
...
- Leakage audit hashed all 907 historical train/validation images and found 2,170 eligible canonical images outside that membership. Frozen group-aware manifests contain exactly 500 images each with zero shared stems, groups, or hashes. Run 1 contains 935 pinhole and 103 scratch objects across 295/62 positive images; run 2 contains 674 pinhole and 81 scratch objects across 285/62 positive images.
- Implemented and tested the exactly-once production-pass cache. Images are decoded before timing; the measured interval covers model preprocessing, synchronized GPU inference, model postprocessing/mask work, prediction conversion, and the frozen confidence/area cascade. Warm-up images are excluded, every measured stem is inferred once, staged timing and raw/kept predictions are written atomically, and completed or partial run directories cannot be silently overwritten.
- Implemented cache-only LabelMe polygon scoring without an Ultralytics/model dependency. It validates cache identity and exact manifest order, merges `inpinhole` into `pinhole`, performs class-aware maximum-IoU one-to-one matching, reconciles TP/FP/FN, writes slipped-defect rows, and reports per-class/overall recall, precision, F1, mean matched mask IoU, support, and 95% Wilson intervals.
- Implemented per-run production timing analysis from the frozen cache: staged/total distribution summaries, exact counts and percentages in fixed `<25`, `25-50`, `50-100`, `100-200`, `200-500`, `500-1000`, and `>=1000 ms` brackets, pinhole-only/scratch-only/mixed/no-target grouping, Spearman relationships with defect presence/object count/prediction count/image dimensions, a Kruskal-Wallis context test, and a two-panel timing graph.
- Implemented independent between-run comparison with Mann-Whitney and Kolmogorov-Smirnov timing tests, Cliff's delta, deterministic bootstrap confidence intervals for mean/median timing differences, timing-bracket and defect-composition chi-square tests, and per-defect two-proportion recall/precision differences with Holm family-wise correction. Interpretation explicitly checks dataset composition before attributing differences to model instability.
- Added the locked experiment runner, manifest/hash validation, completed-run integrity checks, cache-only recovery path, and workspace-local Ultralytics settings directory. The first real-model smoke attempt failed before model load because sandboxed Ultralytics settings targeted the roaming profile; redirecting `YOLO_CONFIG_DIR` to the ignored experiment workspace fixed that environment-only issue without changing weights or experiment configuration.
- Real-model smoke passed on two historical/excluded 2592x2048 images after one excluded warm-up. Exactly two cache/timing rows were written; measured totals were 28.1511 ms and 222.2044 ms, with staged sums reconciling to total time and raw/kept prediction counts recorded. The smoke images are not members of either 500-image manifest.
- Completed both exactly-once production passes: 500 unique images in run 1 and 500 different unique images in run 2. Each run has exactly 500 manifest, cache, and timing rows; cross-run stem/hash/group overlap is zero. All later scoring and comparison reused the frozen caches without model inference.
- Run 1 contained 1,038 target objects and produced TP/FP/FN `833/253/205`, recall 80.25% (95% CI 77.72-82.56%), precision 76.70% (74.10-79.12%), and F1 78.44%. Pinhole was `760/184/175` with 81.28% recall and 80.51% precision; scratch was `73/69/30` with 70.87% recall and 51.41% precision.
- Run 2 contained 755 target objects and produced TP/FP/FN `594/228/161`, recall 78.68% (95% CI 75.61-81.45%), precision 72.26% (69.10-75.21%), and F1 75.33%. Pinhole was `534/167/140` with 79.23% recall and 76.18% precision; scratch was `60/61/21` with 74.07% recall and 49.59% precision.
- Slipped-defect rows exactly equal FN counts: 205 in run 1 and 161 in run 2. No per-defect recall/precision difference between runs remained significant after Holm correction. The historical 83.56% recall / 83.22% precision headline therefore did not reproduce on these new leakage-free samples, and scratch precision remains unsuitable for production.
- Total production-path timing was 50.51 ms mean / 39.94 ms p50 / 115.49 ms p95 / 611.35 ms maximum in run 1 and 44.66 / 37.53 / 92.11 / 236.86 ms in run 2. Fixed timing brackets reconcile to 500 images in each run, and both timing PNGs were generated.
- Run 1 and run 2 had similar broad defect-context composition (`p=0.4092`) but different target-object multiplicity (2.076 versus 1.510 objects/image; Mann-Whitney `p=0.0499`). Typical timing did not shift (Mann-Whitney `p=0.9171`, Cliff's delta `0.0038`), but run 1 had a heavier tail: KS `p=0.0290`, timing-bracket chi-square `p=0.000242`, and mean difference 5.85 ms (bootstrap 95% CI 1.07-11.02 ms).
- Timing correlated most strongly with raw prediction count (Spearman rho 0.971/0.969), then total target-object count (0.835/0.669), pinhole objects (0.778/0.597), and scratch objects (0.365/0.301). Image dimensions were non-significant in run 1 and constant in run 2. Mixed-defect images were slowest on average in both runs.
- Final verification passed all 19 automated tests and independently reconciled manifests, cache identities/hashes, 500 timing rows, bracket totals, TP/FP/FN events, slip rows, graphs, and comparison outputs. The measured report is `aoi_yolo/historical_best_deep_eval/HISTORICAL_BEST_DEEP_EVAL_RESULTS.md`.
